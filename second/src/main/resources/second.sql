...
create database `second` default charset utf8;
...
CREATE TABLE `seller` (
	`id`  INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '表id,自增',
	`name` VARCHAR (50) DEFAULT NULL COMMENT '卖家登录名',
	`password` VARCHAR (32) DEFAULT NULL COMMENT '卖家密码',
	`real_name` VARCHAR (50) DEFAULT NULL COMMENT '卖家真实名字',
	`email` VARCHAR (50) DEFAULT NULL COMMENT '卖家邮箱',
	`phone` VARCHAR (11) DEFAULT NULL COMMENT '卖家电话',
	`created` DATETIME DEFAULT NULL COMMENT '卖家创建时间',
	`updated` DATETIME DEFAULT NULL COMMENT '卖家更新时间',
	PRIMARY KEY (`id`),
	UNIQUE KEY `seller_name_unique`(`name`)
) ENGINE = INNODB AUTO_INCREMENT = 1 DEFAULT CHARSET = utf8 comment '卖家表';

DROP TABLE
IF EXISTS `buyer`;

CREATE TABLE `buyer` (
	`id`  INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '买家表id,自增',
	`name` VARCHAR (50) DEFAULT NULL COMMENT '买家登录名',
	`password` VARCHAR (32) DEFAULT NULL COMMENT '买家密码',
	`real_name` VARCHAR (50) DEFAULT NULL COMMENT '买家真实名字',
	`email` VARCHAR (50) DEFAULT NULL COMMENT '买家邮箱',
	`phone` VARCHAR (11) DEFAULT NULL COMMENT '买家电话',
	`created` DATETIME DEFAULT NULL COMMENT '创建时间',
	`updated` DATETIME DEFAULT NULL COMMENT '更新时间',
	PRIMARY KEY (`id`),
	UNIQUE KEY `buyer_phone_unique`(`phone`),
    unique key `buyer_name_unique`(`name`)
) ENGINE = INNODB AUTO_INCREMENT = 1 DEFAULT CHARSET = utf8 comment '买家表';
...
create table `order`(
    ...
    `buyer_id` int unsigned default null comment '买家id,外键',
    `seller_id` int unsigned default null comment '卖家id,外键',
...
create table `receiver`(
   ...
   `buyer_id` int unsigned default null comment '买家id,来自买家表',
   ...
   primary key(`id`),
   key(`buyer_id`),
   constraint `fk_receiver_buyer` foreign key(`buyer_id`) references `buyer`(`id`)
...
create table `product_image`(
 ...
 create table `leave`(
   ...
   `updated` datetime default null comment '更新时间',
   primary key(`id`),
   key(`product_id`,`buyer_id`),
   constraint `fk_leave_product` foreign key(`product_id`) references `product`(`id`),
   constraint `fk_leave_buyer` foreign key(`buyer_id`) references `buyer`(`id`)
 ...
 create table `releave`(
     ...
     `seller_id` int unsigned default null comment '卖家id,来自卖家表',
   ...
   `updated` datetime default null comment '更新时间',
   primary key(`id`),
   key(`seller_id`,`leave_id`),
   constraint `fk_releave_seller` foreign key(`seller_id`) references `seller`(`id`),
 ...
 create table `deal`(
   ...
   constraint `fk_deal_buyer` foreign key(`buyer_id`) references `buyer`(`id`)
 ...
 create table `collect`(
  ...
  key(`product_id`,`buyer_id`),
   ...
   constraint `fk_collect_buyer` foreign key(`buyer_id`) references `buyer`(`id`)
